...
//            System.out.print(a);
//        }
        Iterator<Amoeba> itr = family.iterator();// traversing over HashSet System.out.println("Traversing over Set using Iterator"); while(itr.hasNext()){ System.out.println(itr.next()); }
        while (itr.hasNext()) {
            Amoeba x = itr.next();
    ...
    public class AmoebaBFSIterator implements Iterator<Amoeba> {
        ...
        public Amoeba next() {
            ...
            Amoeba node = fringe.removeFirst();
            for(int i=0; i < node.children.size(); i++ ){
                if (node.children.get(i) != null) {
                    fringe.addLast(node.children.get(i));
                }
            }
            return node;
        }
