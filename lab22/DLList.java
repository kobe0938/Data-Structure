...
public class DLList<T extends Comparable<T>> {
    ...
    private void insertionSortHelper(T item) {
        ...
        while(currNode.next != sentinel && currNode.next.item.compareTo(item) <= 0){
            currNode = currNode.next;
        }
        Node newNode = new Node(item, currNode, currNode.next);
    ...
    public DLList<T> mergeSort() {
        ...
        for(int i = 0; i < this.size / 2; i++){
            oneHalf.addLast(currNode.item);
            currNode = currNode.next;
        }
        for(int i = this.size/2; i < this.size; i++){
            otherHalf.addLast(currNode.item);
            currNode = currNode.next;
        }
    ...
    public DLList<T> quicksort() {
        ...
        // TODO: YOUR CODE HERE
        for (Node p = this.sentinel.next; p != this.sentinel; p = p.next) {
