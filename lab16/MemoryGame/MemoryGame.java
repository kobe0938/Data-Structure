...
public class MemoryGame {
    ...
    public String generateRandomString(int n) {
        ...
        String randString= "";
        for(int i = 0; i < n; i++){
            int charNum = rand.nextInt(26);
            randString+= CHARACTERS[charNum];
        }
        return randString;
    }
    ...
    public void startGame() {
        ...
        while (!gameOver) {
            ...
            StdDraw.pause(1000);
            String input = generateRandomString(round);
            flashSequence(input);
            String playerGuess = solicitNCharsInput(round);
            playerTurn = false;
            if (playerGuess.equals(input)){
                round++;
            }else{
                gameOver = true;
            }
