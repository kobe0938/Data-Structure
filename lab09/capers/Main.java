...
public class Main {
    ...
    public static void writeStory(String[] args) {
        ...
        String originalString = Utils.readContentsAsString(Story);//story hello hello1
        String currentStory = args[1];//
        String newString = originalString + currentStory + "\n";//
        Utils.writeContents(Story, newString);//
...
//        if(false){
//            exitWithError("You are putting a string rather than a number");
//        }else {
            Dog addDog = new Dog(args[1], args[2], Integer.parseInt(args[3]));
            addDog.saveDog();
            System.out.println(addDog.toString());
    ...
    public static void celebrateBirthday(String[] args) {
        validateNumArgs("birthday", args, 2);//Aday
        Dog bday = Dog.fromFile(args[1]);
        bday.haveBirthday();
        bday.saveDog();
//        Dog.fromFile(args[1]).haveBirthday(); //Bday.age is Aday.age + 1
//        Dog.fromFile(args[1]).saveDog();
    }
