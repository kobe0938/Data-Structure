...
public class SLList {
    ...
    public void add(int index, int x) {
        // TODO
        IntListNode a = this.sentinel.next;
        if (this.size() == 0 || index == 0) {
            this.addFirst(x);
        } else if (this.size() < index) {  // 1 3 5  index=3, index = 2(size)
            for (int i = 0; i < this.size() - 1; i++) {
                a = a.next;
            }
            IntListNode insertone = new IntListNode(x, a.next);
            a.next = insertone;
            size++;
        } else {
    ...
    public IntListNode reverseHelper(IntListNode L) {// 1 2
        //base case
